...
- Encoder(Inference) : 입력 데이터를 내부 표현(잠재 공간)으로 변환 -> 학습할 때 사용
- Decoder(Generation) : Latend Space를 거친 z를 받아 원본 데이터과 같은 형태로 재구성
- Loss(ELBO : Evidence lower bound) : Loss는 크게 두가지를 사용한다. logloss(입력데이터과 출력데이터) & KL divergence(사전분포과 추론분포)를 활용한다. logloss는 생성 모델이기 때문에 Reconstruction error를 줄이기 위해 쓰지만, KL divergence를 활용하는 것이 궁금하다. 사용하는 이유는 Regularization error라고 생각하면 된다. 기존 logloss만 활용하면 데이터 생성시 입력데이터과 같은 값을 출력하려고 하기 때문에 이를 방지하기 위해 정규화를 추가한 것이다. Trajectory로 생각해보면 다음 위치는 이전 위치과 똑같은 위치를 생성하면 안되는 것이다.  

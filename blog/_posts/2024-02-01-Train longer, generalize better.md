...
- 증명은 다루지 않으므로 논문을 참고하기를 바란다.
    ...
    **Random Walk on a Random Potential**
    ...
    - 저자들은 large-batch size에서는 parameter space를 탐색하기 위해 많은 시간이 소요된다. 또한, learning rate를 조정하면 가중치 벡터와의 거리가 상대적으로 비슷한 패턴을 보임을 확인할 수 있다.(small-batch처럼 다양한 parameter space를 탐색이 가능해짐)
    ...
    **Ghost Batch Normalization**
    ...
    - GBN 알고리즘에서 언급되는 $$\epsilon$$은 수치적으로 0이 되지 않도록 하는 매우 작은 값(논문에는 언급이 없음)
    ...
    **Adapting number of weight updates eliminates generalization gap**
    ...
    - 결론적으로 **learning rate를 감소시킬 시점을 결정할 때, validation error가 아닌 weight vector 간의 거리를 활용하는 것이 좋은 기준이 된다고 주장한다. 즉, validation error가 plateau에 도달해도 높은 learning rate를 계속 사용한다는 것이다.**
      ...
      <p align="center">
        ...
        <br>
        weight vector간의 거리 계산 파이썬 코드
      </p> 
      - 위 코드는 weight vector 간의 거리를 계산하는 코드이다. 따로 threshold을 설정하여 이 값을 넘으면 학습률(learning rate)을 줄이는 방식이 있을 줄 알았지만, 실제로는 log에만 기록되는 것 같다. 즉, 직접 log를 확인하여 특정 epoch를 지나면 learing-rate을 줄이는 방식으로 구현해야 할 것으로 보인다.
      ...
      - Learning rate 역시 batch size를 키우면 함께 증가시켜야 한다. 이는 딥러닝의 학습이 초기에는 "ultra-slow diffusion"하기 때문이다. 이를 위해 learning-rate를 다음과 같이 조정해야 한다:
        $$
